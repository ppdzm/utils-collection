package io.github.ppdzm.utils.hadoop.scala.kafka.config
...
object KafkaConsumerProperties {
    ...
    class Builder extends PropertiesBuilder[ConsumerConfig] {
         ...
         * If poll() is not called before expiration of this timeout, then the consumer is considered failed and the group will re-balance in order to reassign the partitions to another member.
         ...
         * If no heartbeats are received by the broker before the expiration of this session timeout, then the broker will remove this client from the group and initiate a re-balance.
